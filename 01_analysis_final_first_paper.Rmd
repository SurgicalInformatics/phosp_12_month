...
source("/home/eharrison/phosp_first_report/geom_boxplot2.R")
```
...
save.image("full_image_210621_first_paper.Rdata", compress = TRUE)
```

```{r}
load("full_image_210621_first_paper.Rdata")
```


## Patients with 3 month and 12 month follow-up

```{r}
# Study id of patients with both 3 month and 12 month recovered
...
### Tier 2 cohort with < 240 days follow-up
...
phosp_hosp %>% 
  ...
  left_join(phosp_6w %>% select(study_id, 
    ...
    left_join(phosp_12m %>% select(study_id, 
                                psq_recovered_12m = psq_recovered,
                                eq5d5l_summary_delta_12m = eq5d5l_summary_delta)) %>% 
  select(study_id, redcap_event_name, starts_with("psq_recovered"))
  mutate(
    psq_recovered = case_when(
      # Use 6 weeks if 3 months missing
      is.na(psq_recovered) & !is.na(psq_recovered_6w) ~ psq_recovered_6w,
      TRUE ~ psq_recovered
    ) %>% 
      
      # Make when recovered. NOTE, this doesn't account for yes at 3 months and no at 12 months
      psq_recovered = case_when(
      psq_recovered == "Yes" ~ "Recovered by 3m",
      psq_recovered_12m == "Yes" ~ "Recovered by 12m",
      psq_recovered_12m == "No" | psq_recovered_12m == "Not sure" ~ "Not recovered",
      TRUE ~ NA_character_
      ) %>% 
      fct_relevel("Not recovered", "", "Recovered by 12m", "Recovered by 3m"),
...
```{r}
phosp_hosp %>%
  mutate(crf3a_bmi = ifelse(is.na(crf3a_bmi), "BMI missing", "BMI complete")) %>% 
  summary_factorlist(dependent, "crf3a_bmi", na_include = TRUE, total_col = TRUE) %>% 
  mytable()
```
...
saveRDS(phosp_hosp_imputed, file = "phosp_hosp_imputed_220621.rds")
```

```{r}
# Load mice sets
phosp_hosp_imputed = readRDS("phosp_hosp_imputed_220621.rds")
# phosp_hosp_imputed = readRDS("phosp_hosp_imputed_020621.rds")
...
```{r eval=FALSE, include=FALSE}
# Extra using age 50 cutoff and interaction
dependent = "psq_recovered"
explanatory = c("age_admission_factor*crf1a_sex", "crf1b_eth_5levels", "imd_quintile", "no_comorbid_3levels", "crf1a_resp_support_4levels", "crf1a_treat_ss", "crf1a_treat_at", "crf1a_treat_tdac")
...
## Table 6C: Multivariable logistic regression (with BMI 2 levels)
...
```{r fig.height=6, fig.width=8}
  phosp_3m %>% 
    ...
    ggplot_lancet(aes(x = symptom_count)) +
    geom_histogram(binwidth = 1, colour = "white", fill = "light blue") + 
    labs(x = "Number of symptoms") +
    ggtitle("Total number of symptoms reported by each individual")
```
...
explanatory_wss = phosp_3m %>% 
  select("patient_sq_l_t__new_disability", 
         matches("patient_sq_l_t_.*_delta_change"), matches("psq_scale_.*_delta_change")) %>%
  names()
...
all_symptoms = c("symptom_any",
                 "symptom_count",
                 cardiorespiratory, gastrointestinal_genitourinary, neuro, musculoskeletal)

dependent = "no_comorbid_2levels"

phosp_3m %>% 
  mutate(across(c(all_symptoms, -c(symptom_count, symptom_any)), ~ fct_relevel(.x, "No"))) %T>%
  {data_in <<- .} %>% 
  summary_factorlist(dependent,  all_symptoms, cont = "median",
                     total_col = TRUE) %>% 
  ff_remove_ref() %>% 
