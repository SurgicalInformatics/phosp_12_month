---
title: "Longitudinal clustering - 4 clusters"
...
paired_study_id = phosp_working %>% 
  ...
  filter(n == 2) %>% 
  pull(study_id)
cat("Pairs prior to imputation, n =", length(paired_study_id))

paired_study_id = phosp_imputed_sets[[1]] %>% 
  count(study_id) %>% 
  filter(n == 2) %>% 
  pull(study_id)
cat("\nPairs after imputation, n =", length(paired_study_id))
```
...
## Linear models (try again with this 21/09)

```{r eval=FALSE}
library(lcmm)
set.seed(1234)
m1_linear = map(1, ~ multlcmm(fixed = gad7_summary + phq9_summary + 
                          pcl5_summary + dyspnoea12_summary + 
                           facit_item_total + sppb_score + mocal_total ~ 
                           
                           time2fu + I(time2fu^2/10), 
                         random = ~ 1, #time2fu, 
                         subject = "study_id2", 
...
saveRDS(m1_linear, file = paste0("m1_linear_", timestamp, ".rds"))

k = 2:6 %>% as.list()
m2_linear = map(k, ~ multlcmm(fixed = gad7_summary + phq9_summary + 
                           pcl5_summary + dyspnoea12_summary + 
                           facit_item_total + sppb_score + mocal_total ~ 
                           
                           time2fu + I(time2fu^2/10), 
                         mixture = ~ time2fu + I(time2fu^2/10),
                         random = ~ 1, #time2fu, 
                         subject = "study_id2", 
                         ...
                         B = m1_linear[[1]], # Use ng = 1 model for initial values
                         data = model_data,
                         maxiter = 100)
)
saveRDS(m2_linear, file = paste0("m2_linear_", timestamp, ".rds"))
m_linear = c(m1_linear, m2_linear)
```

```{r include=FALSE}
m1_linear = readRDS(paste0("m1_linear_", timestamp, ".rds"))
m2_linear = readRDS(paste0("m2_linear_", timestamp, ".rds"))
m_linear = c(m1_linear, m2_linear)
```

```{r}
t1 = summarytable(m_linear[[1]], m_linear[[2]], m_linear[[3]],
             m_linear[[4]], m_linear[[5]], m_linear[[6]],
...
Choose model 4 that has 4 effective clusters. 
...
## Not printed. 
plot(m4)
plot(m4, which="linkfunction")
plot(m4, which="postprob")
plot(m4, which="link")
plot(m4, which="fit", var.time = "time2fu")

datnew <- data.frame(time2fu=seq(0.5, 74, length=100))
m4_linear_predict <- predictY(m4, newdata = datnew, var.time='time2fu', draws=TRUE) 

plot(m4_linear_predict, which="fit", var.time = "time2fu")
```
...
set.seed(1234)
# 10 sets of model 4
m3_linear_all_imputed = model_data_imputed %>% 
  map(~ multlcmm(fixed = gad7_summary + phq9_summary + 
                   ...
                   time2fu + I(time2fu^2/10), 
                 mixture = ~ time2fu + I(time2fu^2/10),
                 random = ~ 1, #time2fu, NOTE THIS CHANGE
                 subject = "study_id2", 
...
saveRDS(m3_linear_all_imputed, file = paste0("m3_linear_all_imputed", timestamp, ".rds"))
```

```{r}
m3_linear_all_imputed = readRDS(paste0("m3_linear_all_imputed", timestamp, ".rds"))
m4_linear_all_imputed = readRDS(paste0("m4_linear_all_imputed", timestamp, ".rds"))

# CHOOSE HERE
m4_linear_all_imputed = m3_linear_all_imputed
```

```{r}
# Metrics of models developed with imputed sets
# 3 cluster model
summarytable(m3_linear_all_imputed[[1]], m3_linear_all_imputed[[2]],
             m3_linear_all_imputed[[3]], m3_linear_all_imputed[[4]],
             m3_linear_all_imputed[[5]], m3_linear_all_imputed[[6]],
             m3_linear_all_imputed[[7]], m3_linear_all_imputed[[8]],
             m3_linear_all_imputed[[9]], m3_linear_all_imputed[[10]],
             which =c( "G", "loglik", "conv", "npm",
                       "AIC", "BIC", "SABIC", 
                       "entropy", "ICL", "%class")) 

# 4 cluster model
summarytable(m4_linear_all_imputed[[1]], m4_linear_all_imputed[[2]],
             m4_linear_all_imputed[[3]], m4_linear_all_imputed[[4]],
             m4_linear_all_imputed[[5]], m4_linear_all_imputed[[6]],
             m4_linear_all_imputed[[7]], m4_linear_all_imputed[[8]],
             m4_linear_all_imputed[[9]], m4_linear_all_imputed[[10]],
             which =c( "G", "loglik", "conv", "npm",
                       "AIC", "BIC", "SABIC", 
                       "entropy", "ICL", "%class")) 
...
m4_linear_all_imputed_class = m4_linear_all_imputed %>% 
  ...
  bind_cols(study_id2 = m4_linear_all_imputed[[1]]$pprob[, c(1)], 
            .) %>% 
  tibble() %>% 
...
```{r}
# Choose beta or linear
# Comment out for beta
m5_beta_all_imputed_class = m4_linear_all_imputed_class
...
phosp_m5 %>% distinct(study_id, .keep_all = TRUE) %>% count(class)
```
...
phosp_utility = phosp_m5 %>% 
  ...
  left_join(phosp_m5 %>% select(study_id, class) %>% 
              distinct(study_id, .keep_all = TRUE) %>% 
              drop_na()) %>% 
  ungroup()
...
```{r}
phosp_utility %>% distinct(study_id, .keep_all = TRUE) %>% count(class)
```
...
phosp_utility %>% 
    ...
    mutate(redcap_event_name = factor(redcap_event_name,
                                      levels = c("Pre-COVID", "3 Months (1st Research Visit)", 
                                                 "12 Months (2nd Research Visit)"))) %T>%
  {plot_data <<- count(., class, redcap_event_name)} %>% 
  ggplot_lancet(aes(x = class, y = eq5d5l_utility_index, fill = class, group = interaction(redcap_event_name, class))) + 
  geom_boxplot() +
  geom_text(data = plot_data, aes(x = class, y = -0.5, label = n),
            position = position_dodge(width = 0.8), hjust = 0.5) +
  scale_colour_brewer("Cluster", palette = "Set4") +
  scale_fill_brewer("Event", palette = "Set1") +
  labs(x = "Cluster", y = "EQ5D5l Utility Index") +
  theme(legend.position = "none") +
  ggtitle("Patient symptom cluster by EQ5Dl utility index",
        "Each group: Pre-Covid, 6 months, 12 months | Numbers are counts")
...
```{r}
phosp_utility %>% 
  count(class, redcap_event_name)
```


## 12 month regression model

### Multivariable logistic regression (complete cases, no clustering by hospital)
...
## Figure: Multivariable logistic regression
